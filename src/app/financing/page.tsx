...
export default function FinancingSankeyPage() {
  ...
  useEffect(() => {
    ...
    linkGrad.append('stop').attr('offset', '0%').attr('stop-color', LINK_COLOR_DARK)
    linkGrad.append('stop').attr('offset', '100%').attr('stop-color', LINK_COLOR_LIGHT)
    ...
    d3.json('public/data/financing_sankey.json').then((data: unknown) => {
      const d = data as { nodes: { name: string; level: number }[]; links: { source: number; target: number; value: number }[] }
      if (!d) return
      ...
      labelG.filter((d: unknown) => (d as { level: number }).level === 0)
        .each(function(d: unknown) {
          const n = d as { name: string; value: number; x0: number; y0: number; y1: number }
      ...
      labelG.filter((d: unknown) => (d as { level: number }).level === 1)
        .each(function(d: unknown) {
          const n = d as { name: string; value: number; x0: number; x1: number; y0: number }
      ...
      labelG.filter((d: unknown) => (d as { level: number }).level === 2)
        .each(function(d: unknown) {
          const n = d as { name: string; value: number; x0: number; x1: number; y0: number; y1: number }
          const xMid = (n.x0 + n.x1) / 2
          const yMid = (n.y0 + n.y1) / 2
          const sel = d3.select(this)
      ...
      labelG.filter((d: unknown) => (d as { level: number }).level === 3)
        .each(function(d: unknown) {
          const n = d as { name: string; value: number; x1: number; y0: number; y1: number }
          const yMid = (n.y0 + n.y1) / 2
          const sel = d3.select(this)
  ...
  return (
    <div style={{ width: '100%', height: '100vh', background: ORANGE, overflow: 'hidden' }}>
