...
export default function PartnersVizClient({ initialNodes }: { initialNodes: HexNode[] }) {
  ...
  return (
    ...
    {ordered.map((n) => {
  const dim = hoveredLabel !== null && n.kind !== "outline" && n.kind !== "center";
  const isSameGroup =
    hoveredLabel !== null &&
  ...
  return (
    <g
      key={n.id}
      onMouseEnter={
        n.kind === "label" ? () => setHoveredLabel(n.label ?? null) : undefined
      }
      onMouseLeave={n.kind === "label" ? () => setHoveredLabel(null) : undefined}
