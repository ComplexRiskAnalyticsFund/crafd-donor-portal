...
export default function CoverageMap({ coverage }: { coverage: string | null }) {
  ...
  useEffect(() => {
    ...
    async function draw() {
      ...
      const lookup = coverage != null ? COVERAGE_COUNTRY_IDS[coverage] : undefined;
      ...
      const countries = topojson.feature(geoCache, geoCache.objects.countries) as unknown as GeoJSON.FeatureCollection;
      ...
      if (highlightIds === null) {
        ...
        if (highlightIds.size > 0) {
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const highlightGeoms = (geoCache.objects.countries.geometries as any[]).filter(
