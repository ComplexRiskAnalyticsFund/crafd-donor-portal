...
OUTPUT_SANKEY_JSON = PUBLIC_DATA_DIR / "financing_sankey.json"
...
all_data = consolidating_data([INPUT_PARTNERS_JSON, INPUT_PROJECTS_JSON, INPUT_DONORS_JSON])
...
projects_df['grant_amt'] = projects_df['grant_amt'].replace({'\$': '', ',': ''}, regex=True).astype(int)
...
def match_org_types(row):
    for _, lead in partners_df_leads.iterrows():
        if re.search(r'\(' + re.escape(lead['org_short_name']) + r'\)', row['project_lead_org']):
...
nodes = list(set(sankey_df['project_investment_type']).union(set(sankey_df['org_type'].dropna())))
...
grouped_flows_all = sankey_df.groupby(['project_investment_type', 'org_type'], as_index=False)['project_grant_amt'].sum()
...
org_grouped_df = sankey_df.groupby('org_type', as_index=False)['project_grant_amt'].sum()
