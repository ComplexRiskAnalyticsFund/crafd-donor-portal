...
df_parners_registry = fetch_airtable_table(
    ...
    base_id=DATAVERSE_AIRTABLE_BASE_ID,
)
...
df_crisis_data_funding_compass = fetch_airtable_table(
    table_id=CRISIS_DATA_FUNDING_COMPASS_TABLE_ID,
    base_id=FUNDING_COMPASS_AIRTABLE_BASE_ID,
)
...
df_parners_registry["org_key"] = (
    df_parners_registry["Short name"]
    .str.strip()
    .str.lower()
...
matches2 = (
    useful_columns["org_full_name"]
    .str.strip()
    .str.lower()
    .str.replace(r"\s+", "-", regex=True)
    .isin(
        df_parners_registry["Organization name"]
        .str.strip()
        .str.lower()
        .str.replace(r"\s+", "-", regex=True)
    )
...
merged_df = useful_columns[
    useful_columns["org_key"].isin(df_parners_registry["org_key"])
].copy()

# Add partner registry columns to the merged DataFrame
merged_df = merged_df.merge(
    df_parners_registry[["org_key", "Organization Type", "UN-Organization"]],
    on="org_key",
    how="left",
